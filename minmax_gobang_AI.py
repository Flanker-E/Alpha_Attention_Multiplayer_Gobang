...
def ai(is_ai):
    ...
    global search_count   
    search_count = 0
    negamax(is_ai, DEPTH, -99999999, 99999999)
...
def negamax(is_ai, depth, alpha, beta):
    ...
    if game_win(list0) or game_win(list1) or game_win(list2) or depth == 0:
    ...
    for next_step in blank_list:
        ...
        value = -negamax(2-is_ai, depth - 1, -beta, -alpha)
        if is_ai == 0:
            list0.remove(next_step)
        elif is_ai == 1:
...
def evaluation(is_ai):
    ...
    if is_ai == 0:
        my_list = list0
        enemy_list = list1 + list2
    elif is_ai == 1:
        my_list = list1
        enemy_list = list0 + list2
    else:
        my_list = list2
        enemy_list = list0 + list1
...
def go():
    ...
    while g == 0:
        ...
        if change % 3 == 1:
            ...
            if pos in list3:
                message = Text(Point(200, 200), "unavailable location" + str(pos[0]) + "," + str(pos[1]))
                message.draw(win)
                g = 1
            ...
            if game_win(list1):
                message = Text(Point(100, 100), "green win.")
                message.draw(win)
                g = 1
            change = change + 1
        else:

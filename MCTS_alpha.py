...
class MCTSPlayerAlpha(MCTSPlayer):
    def __init__(self,**kwargs) -> None:
        ...
        if who_play=='alpha_MCTS':
            ...
            self.mcts=MCTSAlpha(c_puct, n_playout, num_player,policy_value_fn=policy_value_fn)
...
class MCTSAlpha(MCTS):
    def __init__(self, c_puct, n_playout, num_player, policy_value_fn=None) -> None:
        super(MCTSAlpha,self).__init__(c_puct, n_playout, num_player, policy_value_fn)
    ...
    def _playout(self,board_state):
        ...
        cur_node.update_recursive(winner,(cur_player+(self.num_player-1))%self.num_player, leaf_val=leaf_val)
        return cur_node
    ...
    def get_move(self, board_state, temp=1e-3):
        ...
        act_prob=[(act,node._n_visits) for act, node in self._root.children. items()]
